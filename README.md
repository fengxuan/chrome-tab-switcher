...
- 使用收藏夹快捷键 `Command + Shift + F`（Windows/Linux 默认是 `Ctrl + Shift + F`）直接打开收藏夹视图；
...
如果希望在 Excel、VS Code 等其它应用中也能唤出切换器，可以运行父目录 `macos/ChromeTabSwitcherHotKey` 中的本地辅助程序。它监听系统级 `Command + Shift + Space` 打开普通切换器，也监听 `Command + Shift + F` 直接打开收藏夹视图，激活最近使用的 Chrome 窗口后把对应快捷键发送给扩展。Chrome 没有运行时，辅助程序会尝试启动 Chrome。启动后，辅助程序会显示在 macOS 菜单栏中。
...
首次运行后，请在“系统设置 → 隐私与安全性 → 辅助功能”中允许“Chrome Tab Switcher”。这是 macOS 对跨应用发送键盘事件的权限要求。也可以从菜单栏中的“打开辅助功能设置”，或“更换全局快捷键”窗口中的“打开辅助功能设置”按钮直接跳转到授权页面。菜单栏中会显示普通切换器和收藏夹快捷键；可以打开切换器、打开收藏夹、查看监听状态、更换普通切换器快捷键或退出程序。更换普通快捷键后，需要同时在 `chrome://extensions/shortcuts` 中将扩展的普通切换器快捷键设置成相同组合；收藏夹默认快捷键为 `⌘⇧F`。录入新快捷键时，辅助程序会暂时停止监听快捷键，保存或取消后自动恢复。之后可以把 `dist/Chrome Tab Switcher.app` 加到“系统设置 → 通用 → 登录项”，让它开机常驻。
...
- `Esc`：关闭面板
- `⌘ K` / `Ctrl K`：聚焦搜索框
- 点击搜索框左侧的星标 `Favorites` 按钮：只显示收藏夹中的书签标签页，并按收藏夹目录分组；在此模式下搜索只匹配当前收藏夹列表。点击收藏夹卡片时，如果网址已打开则切换到对应标签页，否则打开该网址。收藏夹图标会优先复用已打开标签页的 favicon；未打开时只请求一次网站根目录的 `/favicon.ico`，失败时显示首字母。
- 在 `chrome://extensions/shortcuts` 中可以修改两个扩展快捷键；收藏夹快捷键会直接进入收藏夹视图。若使用 macOS 辅助程序，需让收藏夹命令保持默认的 `⌘⇧F`，辅助程序会监听并转发这个快捷键。
...
扩展使用 `tabs` 和 `windows` 权限来读取跨窗口的标签标题、网址和网站图标，使用 `bookmarks` 权限读取收藏夹目录和书签，使用 HTTP/HTTPS 主机权限在收藏夹条目缺少 favicon 时请求一次网站根目录的 `/favicon.ico`，使用 `storage.session` 保存切换器窗口和最近标签，以便 Manifest V3 service worker 重启后恢复。favicon 请求只获取图标文件，不读取网页内容；其它标签信息和网页内容仍只在本机处理，不上传到第三方服务。
