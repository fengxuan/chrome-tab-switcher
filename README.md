...
- `Esc`：关闭面板
- `⌘ K` / `Ctrl K`：聚焦搜索框
- 点击搜索框左侧的星标 `Favorites` 按钮：只显示收藏夹中的书签标签页，并按收藏夹目录分组；在此模式下搜索只匹配当前收藏夹列表。点击收藏夹卡片时，如果网址已打开则切换到对应标签页，否则打开该网址。收藏夹图标会优先复用已打开标签页的 favicon；未打开且没有可用图标时显示首字母。
...
扩展使用 `tabs` 和 `windows` 权限来读取跨窗口的标签标题、网址和网站图标，使用 `bookmarks` 权限读取收藏夹目录和书签，使用 `storage.session` 保存切换器窗口和最近标签，以便 Manifest V3 service worker 重启后恢复。拼音字典基于 MIT 许可的 pinyinjs，并随扩展本地打包；所有代码都在本地运行，不会上传标签信息或网页内容。
