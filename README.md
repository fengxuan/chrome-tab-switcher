...
- 会在本机保留最近 7 天访问过的网页元数据；关闭窗口后，搜索仍可在底部推荐并重新打开这些网页；
- 可选的智能休眠会释放长时间未访问标签页的页面资源，同时保留标签页记录；
- 支持鼠标、键盘和全局快捷键切换；
...
- 仅显示 macOS 原生应用：`Command + Shift + Y`（Windows/Linux 为 `Ctrl + Shift + Y`，无 macOS 应用时为空）。
...
菜单栏中的“智能休眠设置…”会直接打开切换器底部的智能休眠面板；也可以使用上面的专用快捷键。
...
- 普通标签卡片选中时会显示左上角星标，点击可直接收藏当前标签页；已收藏的标签会显示实心星标；未选中时，状态点显示在最左上角；
- 点击底部的“智能休眠”可开启自动休眠，并选择 30 分钟、1/2/4/8 小时的空闲时间；macOS 也可以从菜单栏中的“智能休眠设置…”直接打开这个面板；休眠标签页会显示灰色角标，点击后由 Chrome 自动重新加载；
- 点击星标 `Favorites`：只查看收藏夹；点击收藏卡片右上角的星标可取消收藏。
- 收藏夹视图中的“最近访问”标记和筛选：显示最近 30 天内访问过的书签；这些书签会按最近访问时间优先排列。

在普通视图中输入搜索词时，切换器会在底部单独显示最近 7 天内访问过、但当前已经关闭的匹配网页。点击历史推荐会在现有 Chrome 窗口中新建标签页；历史记录只保存 URL、标题、favicon、访问时间和次数，不保存网页正文或表单会话。历史推荐不会与当前打开的标签页混在同一组中。
...
在 macOS 上，打开 Chrome 标签页、收藏夹或原生应用视图时，搜索输入会优先切换到英文输入源；关闭切换器后恢复之前的输入源。该功能需要 macOS Helper 正在运行；未安装 Helper 时仍可使用原有搜索功能。
...
- `bookmarks`：读取收藏夹；
- `storage`：在本机保存最近 7 天的访问元数据、收藏夹访问统计和智能休眠设置；
- `alarms`：在后台定时检查是否有达到空闲时间的标签页；
- `scripting` 和 HTTP/HTTPS 内容脚本：仅在启用智能休眠时本地检查表单是否正在填写，以避免丢失输入；
- `nativeMessaging`：通过本机 Swift Helper 读取和激活 macOS 窗口；
- HTTP/HTTPS host 权限：仅在收藏夹缺少 favicon 时请求网站根目录的 `/favicon.ico`。

macOS Helper 使用 Accessibility API 读取窗口标题、窗口位置并激活具体窗口。标签页信息、历史元数据、窗口信息和网页内容不会上传到第三方服务。历史推荐点击后只恢复 URL，无法恢复已关闭标签页中的表单输入、滚动位置或页面会话状态。
