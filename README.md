...
- 鼠标移到普通标签卡片时，点击左上角的星标可直接收藏当前标签页；已收藏的标签会显示实心星标；
- 点击底部的“智能休眠”可开启自动休眠，并选择 30 分钟、1/2/4/8 小时的空闲时间；休眠标签页会显示灰色角标，点击后由 Chrome 自动重新加载；
- 点击星标 `Favorites`：只查看收藏夹；点击收藏卡片右上角的星标可取消收藏。
- 收藏夹视图中的“最近访问”标记和筛选：显示最近 30 天内访问过的书签；这些书签会按最近访问时间优先排列。

在普通视图中输入搜索词时，切换器会在底部显示最近 7 天内访问过、但当前已经关闭的匹配网页。点击历史推荐会在现有 Chrome 窗口中新建标签页；历史记录只保存 URL、标题、favicon、访问时间和次数，不保存网页正文或表单会话。
