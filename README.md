...
- `Esc`：关闭面板
- `⌘ K` / `Ctrl K`：聚焦搜索框
- 绿色小点：最近切换的 4 个标签页；右上角动画播放按钮：正在播放音频或视频的标签页
...
扩展使用 `tabs` 和 `windows` 权限来读取跨窗口的标签标题、网址和网站图标，使用 `storage.session` 保存切换器窗口和最近标签，以便 Manifest V3 service worker 重启后恢复。拼音字典基于 MIT 许可的 pinyinjs，并随扩展本地打包；所有代码都在本地运行，不会上传标签信息或网页内容。
